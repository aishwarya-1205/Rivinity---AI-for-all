...
const SettingsModal = ({ isOpen, onClose }: SettingsModalProps) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedTheme = localStorage.getItem("rivinity-theme") as Theme;
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {themeOptions.map((t) => (
                        ...
                        >
                          ...
                          {theme === t.value && <Check className="w-3 h-3 text-accent" />}
                        </button>
                      ))}
                    </div>
                  </div>
...
const ToggleRow = ({
  ...
  <div className="flex items-center justify-between gap-4 p-3 rounded-xl bg-white/50 dark:bg-white/5 border border-border/10">
    ...
    <button
      ...
      className={`w-9 h-5 rounded-full transition-all duration-200 relative shrink-0 ${value ? "bg-accent/60" : "bg-muted-foreground/20"}`}
    >
      <span
