      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
          ...
          >
            ...
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-background border border-border flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-accent border border-accent/30" />
              </div>
              {/* Year label */}
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              {/* Mobile year */}
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-muted-foreground/30">
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-highlight via-accent to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
