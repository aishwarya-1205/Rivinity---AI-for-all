        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[440px]">
          ...
          <div className="flex flex-col gap-4">
            <div className="glass rounded-2xl border border-glass p-5 flex-1 flex flex-col">
              ...
              >
                <p className="text-[11px] font-medium text-accent">Add New Sample</p>
              </div>
              ...
              {clonedVoices.map((v) => (
                <div key={v.id} className="p-3 rounded-xl bg-muted/20 border border-glass/10 hover:bg-muted/40 transition-all group">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-accent/60 font-bold uppercase">{v.quality}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-muted-foreground/50">{v.samples} samples · {v.created}</span>
                    <button className="text-[10px] font-bold text-accent opacity-0 group-hover:opacity-100 transition-opacity">USE THIS</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl border border-glass shadow-float p-5">
                ...
                {genderDropdownOpen && (
                  <div className="absolute top-full mt-2 left-0 right-0 z-50 glass-strong rounded-xl border border-glass shadow-float p-2 animate-float-in min-w-[280px]">
                    ...
                    <div className="grid grid-cols-1 gap-1">
                      {genderOptions.map(g => (
                        <button key={g.id} onClick={() => { setSelectedGender(g); setGenderDropdownOpen(false); }} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] transition-all ${g.id === selectedGender.id ? 'bg-accent/20 text-accent' : 'hover:bg-accent/40'}`}>
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={s.label}>
                          ...
                          <input type="range" min="0" max="100" value={s.value} onChange={e => s.set(Number(e.target.value))} className="w-full h-1 accent-accent" />
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-transparent border border-glass/10 rounded-xl text-[13px] leading-relaxed text-foreground/80 placeholder:text-muted-foreground/20 focus:outline-none focus:border-accent/30 px-4 py-3 resize-none mb-4"
            />
            ...
            <div className="glass rounded-2xl border border-glass p-5 animate-float-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className={`p-3 rounded-xl border ${p.gradient ? 'bg-accent/5 border-accent/10' : 'bg-muted/30 border-glass/10'}`}>
                    <p className={`text-[9px] uppercase font-bold mb-3 ${p.gradient ? 'text-accent' : 'text-muted-foreground/40'}`}>{p.label}</p>
                    <div className="flex items-center gap-3">
                      <button
