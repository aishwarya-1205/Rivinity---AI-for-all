            ...
            <div className="absolute top-full mt-2 right-0 z-50 glass-strong rounded-xl border border-glass shadow-float overflow-hidden animate-float-in min-w-[200px] p-2">
              ...
              <div className="border-t border-glass/10 pt-2 px-2">
                ...
                >
                  Speaker Detection
                  <div
                    className={`w-7 h-4 rounded-full relative transition-colors ${diarize ? "bg-accent" : "bg-muted-foreground/20"}`}
                  >
                    <div
          ...
          <div className="py-2">
            {uploadedFile ? (
              ...
              <div
                ...
                className="border-2 border-dashed border-glass/20 rounded-xl p-10 text-center cursor-pointer hover:border-accent/40 hover:bg-accent/20 transition-all group"
              >
                <Upload className="w-7 h-7 text-muted-foreground/20 mx-auto mb-3 group-hover:text-accent transition-colors" />
        ...
        <div className="glass rounded-2xl border border-glass shadow-float overflow-hidden animate-float-in flex flex-col">
          ...
          <div className="flex justify-center gap-1.5 pb-6">
            {sampleTranscript.map((_, i) => (
              <div
                key={i}
                className={`h-1 rounded-full transition-all duration-300 ${i === activeSpeechIndex ? "w-6 bg-accent" : "w-1.5 bg-muted-foreground/20"}`}
              />
            ))}
          </div>
        </div>
