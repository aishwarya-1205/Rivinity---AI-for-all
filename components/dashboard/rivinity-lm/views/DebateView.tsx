            ...
            {debateTopics.map(t => (
              <button key={t} onClick={() => setTopic(t)}
                className={`w-full text-left px-4 py-3 rounded-xl text-[13px] transition-all shrink-0 ${
                  topic === t ? "glass border border-accent/20 shadow-glow-accent text-foreground/80 font-medium" : "text-muted-foreground/60 hover:bg-accent/50"
                }`}
              >{t}</button>
            ))}
          </div>
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`group flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
              ...
              {msg.feedback && (
                <div className="mt-3 pt-3 border-t border-glass">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-semibold text-accent/70">Score: {msg.score}/10</span>
                  </div>
              ...
              {msg.role === "ai" && (
                <div className="flex items-center gap-1 mt-1 ml-1 text-muted-foreground/40 opacity-0 group-hover:opacity-100 transition-opacity">
