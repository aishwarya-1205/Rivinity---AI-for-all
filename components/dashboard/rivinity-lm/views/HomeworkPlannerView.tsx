...
const statusColors = { pending: "bg-muted text-muted-foreground/60", "in-progress": "bg-accent/10 text-accent", done: "bg-green-500/10 text-green-600" };
        ...
        <div className="glass rounded-2xl border border-glass p-5 mb-6 animate-float-in shadow-float">
          <div className="flex gap-3 mb-3">
            <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="What's the assignment?"
              className="flex-1 bg-muted/30 rounded-xl px-4 py-2.5 text-[12px] text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-accent/20" />
            <input value={newSubject} onChange={(e) => setNewSubject(e.target.value)} placeholder="Subject"
              className="w-[140px] bg-muted/30 rounded-xl px-4 py-2.5 text-[12px] text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-accent/20" />
          </div>
          <div className="flex items-center justify-between">
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-accent bg-accent/10 hover:bg-accent/15 transition-colors">
            ...
            <div className="flex items-start gap-3">
              <button onClick={() => toggleStatus(task.id)}
                className={`w-5 h-5 mt-0.5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${
                  task.status === "done" ? "bg-green-500 border-green-500" : "border-muted-foreground/20 hover:border-accent/40"
                }`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {task.aiSuggestion && task.status !== "done" && (
                  <div className="mt-2 flex items-start gap-2 bg-accent/5 rounded-lg p-2.5">
